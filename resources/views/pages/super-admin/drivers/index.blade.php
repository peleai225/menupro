    ...
    <div class="space-y-6">
            ...
            <div class="rounded-2xl border p-5 shadow-sm transition hover:shadow-md" style="border-color:var(--sa-border);background:var(--sa-card);">
                <div class="flex items-start justify-between">
                    <span class="flex size-11 items-center justify-center rounded-xl" style="background:color-mix(in oklch,var(--sa-success) 10%,transparent);color:var(--sa-success);">
                        <svg class="size-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    </span>
                </div>
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
            ...
            <form method="GET" class="flex items-center gap-2">
                ...
                <div class="relative">
                    <svg class="absolute left-3 top-1/2 -translate-y-1/2 size-4 pointer-events-none" style="color:var(--sa-muted-fg);" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-4.35-4.35"/></svg>
                    ...
                    <tr class="transition-colors" style="border-bottom:1px solid var(--sa-border);"
                        ...
                        <td class="px-5 py-3.5">
                            <div class="flex items-center gap-3">
                                <span class="flex size-9 items-center justify-center rounded-xl text-sm font-bold text-white flex-shrink-0"
                        ...
                        <td class="px-5 py-3.5">
                            @if($driver->rating)
                                <span class="inline-flex items-center gap-1 font-medium" style="color:var(--sa-fg);">
                                    <svg class="size-4" style="fill:var(--sa-warning);color:var(--sa-warning);" viewBox="0 0 24 24">
                        ...
                        <td class="px-5 py-3.5">
                            @if($driver->verification_status === 'approved' && $driver->is_active && $driver->is_available)
                                <span class="inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-xs font-medium whitespace-nowrap"
                                    ...
                                    <span class="size-1.5 rounded-full bg-current"></span>Disponible
                                </span>
                            @elseif($driver->verification_status === 'approved' && $driver->is_active)
                                <span class="inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-xs font-medium whitespace-nowrap"
                                    ...
                                    <span class="size-1.5 rounded-full bg-current"></span>En course
                                </span>
                            @elseif($driver->verification_status === 'pending')
                                <span class="inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-xs font-medium whitespace-nowrap"
                                    ...
                                    <span class="size-1.5 rounded-full bg-current"></span>En attente
                                </span>
                            @elseif($driver->verification_status === 'rejected')
                                <span class="inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-xs font-medium whitespace-nowrap"
                                    ...
                                    <span class="size-1.5 rounded-full bg-current"></span>Rejeté
                                </span>
                            @else
                                <span class="inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-xs font-medium whitespace-nowrap"
                                    ...
                                    <span class="size-1.5 rounded-full bg-current"></span>Hors ligne
                                </span>
                            @endif
                        </td>
                        <td class="px-5 py-3.5">
                                ...
                                @elseif($driver->is_active)
                                    <form method="POST" action="{{ route('super-admin.drivers.suspend', $driver) }}">@csrf
                                        <button type="submit" class="p-1.5 rounded-lg transition-colors"
                                                ...
                                                onmouseout="this.style.background='transparent'"
                                                title="Suspendre">
                                            <svg class="size-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 9v6m4-6v6"/></svg>
                                        </button>
                                    </form>
                                @else
                                    <form method="POST" action="{{ route('super-admin.drivers.reactivate', $driver) }}">@csrf
                                        <button type="submit" class="p-1.5 rounded-lg transition-colors"
                                                ...
                                                onmouseout="this.style.background='transparent'"
                                                title="Réactiver">
                                            <svg class="size-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z"/></svg>
                                        </button>
                                    </form>
                                @endif
                            </div>
