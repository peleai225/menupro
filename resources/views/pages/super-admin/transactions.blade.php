<x-layouts.admin-super title="Transactions">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
            <h1 class="text-2xl font-bold text-white">Transactions & Paiements</h1>
            <p class="text-neutral-400 mt-1">Suivi de tous les flux financiers de la plateforme.</p>
        </div>
        <a href="{{ route('super-admin.transactions.export') }}" class="btn btn-outline border-neutral-600 text-neutral-300 hover:bg-neutral-700">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div class="bg-gradient-to-br from-secondary-500/20 to-secondary-600/10 border border-secondary-500/30 rounded-2xl p-5">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-secondary-300">Revenus totaux</p>
                    <p class="text-2xl font-bold text-white mt-1">{{ number_format($stats['total_revenue'], 0, ',', ' ') }} F</p>
                </div>
        ...
        <div class="bg-neutral-800/50 border border-neutral-700 rounded-2xl p-5">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-neutral-400">Ce mois</p>
                    <p class="text-2xl font-bold text-white mt-1">{{ number_format($stats['this_month_revenue'], 0, ',', ' ') }} F</p>
                </div>
        ...
        <div class="bg-neutral-800/50 border border-neutral-700 rounded-2xl p-5">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-neutral-400">Abonnements</p>
                    <p class="text-2xl font-bold text-blue-400 mt-1">{{ number_format($stats['subscriptions_revenue'], 0, ',', ' ') }} F</p>
                </div>
        ...
        <div class="bg-neutral-800/50 border border-neutral-700 rounded-2xl p-5">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-accent-400 mt-1">{{ number_format($stats['orders_revenue'], 0, ',', ' ') }} F</p>
                </div>
    ...
    <!-- Chart -->
    <div class="bg-neutral-800/50 border border-neutral-700 rounded-2xl p-6 mb-8">
        <h3 class="text-lg font-semibold text-white mb-4">Revenus des 30 derniers jours</h3>
    ...
    <form method="GET" class="bg-neutral-800/50 border border-neutral-700 rounded-xl p-4 mb-6">
        <div class="flex flex-col lg:flex-row gap-4">
            ...
            <select name="type" class="h-10 px-4 bg-neutral-700 border border-neutral-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-primary-500">
    ...
    <!-- Transactions Table -->
    <div class="bg-neutral-800/50 border border-neutral-700 rounded-2xl overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead class="bg-neutral-900/50 border-b border-neutral-700">
                    <tr>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-neutral-400 uppercase">ID</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-neutral-400 uppercase">Type</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-neutral-400 uppercase">Restaurant</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-neutral-400 uppercase">Description</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-neutral-400 uppercase">Montant</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-neutral-400 uppercase">Méthode</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-neutral-400 uppercase">Statut</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-neutral-400 uppercase">Date</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-neutral-700/50">
                    @forelse($transactions as $transaction)
                        <tr class="hover:bg-neutral-700/30 transition-colors">
                            <td class="px-6 py-4">
                                <span class="font-mono text-sm text-neutral-300">{{ $transaction['id'] }}</span>
                            </td>
                            <td class="px-6 py-4">
                                @php
                                    $typeColors = [
                            ...
                            <td class="px-6 py-4">
                                @if($transaction['restaurant'])
                                    <a href="{{ route('super-admin.restaurants.show', $transaction['restaurant']) }}" class="text-white hover:text-primary-400 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <span class="text-neutral-300">{{ $transaction['description'] }}</span>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                <span class="text-neutral-400 text-sm">{{ $transaction['payment_method'] }}</span>
                            </td>
                            <td class="px-6 py-4">
                                @php
                                    $statusColors = [
                            ...
                            <td class="px-6 py-4">
                                <span class="text-neutral-400 text-sm">{{ $transaction['created_at']->format('d/m/Y H:i') }}</span>
                            </td>
                        </tr>
                    @empty
                        <tr>
                ...
                <div class="flex items-center gap-2">
                    @if($pagination['current_page'] > 1)
                        <a href="?page={{ $pagination['current_page'] - 1 }}&{{ http_build_query($filters) }}" 
                           class="px-3 py-1.5 bg-neutral-700 text-white rounded-lg hover:bg-neutral-600 transition-colors">
                            Précédent
                        </a>
                    @endif
