<x-layouts.admin-super title="Client : {{ $customer->user->name ?? '—' }}">
    <div class="space-y-6 max-w-6xl">
        ...
        <div class="flex items-center gap-2 text-sm text-neutral-500">
            <a href="{{ route('super-admin.customers.index') }}" class="hover:text-neutral-700">Clients</a>
            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
            <span class="text-neutral-900 font-medium">{{ $customer->user->name ?? '—' }}</span>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-neutral-200 shadow-sm p-6">
            <div class="flex flex-wrap items-center justify-between gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-xl font-bold text-neutral-900">{{ $customer->user->name ?? '—' }}</h1>
                        <p class="text-sm text-neutral-500">{{ $customer->user->email ?? '—' }}</p>
                        <div class="mt-1">
                ...
                <a href="{{ route('super-admin.customers.index') }}"
                   class="inline-flex items-center gap-2 h-9 px-4 bg-white border border-neutral-200 rounded-xl text-sm font-medium text-neutral-600 hover:bg-neutral-50 hover:text-neutral-800">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div class="bg-white rounded-2xl p-4 border border-neutral-200 shadow-sm">
                <p class="text-xs text-neutral-500">Total commandes</p>
                <p class="text-2xl font-bold text-neutral-900 mt-1">{{ $stats['total_orders'] }}</p>
            </div>
            <div class="bg-white rounded-2xl p-4 border border-neutral-200 shadow-sm">
                <p class="text-xs text-neutral-500">Total dépensé</p>
                <p class="text-2xl font-bold text-emerald-600 mt-1">{{ number_format($stats['total_spent']) }} F</p>
            </div>
            <div class="bg-white rounded-2xl p-4 border border-neutral-200 shadow-sm">
                <p class="text-xs text-neutral-500">Dernière commande</p>
                <p class="text-lg font-bold text-neutral-900 mt-1">
                    ...
                    @else
                        <span class="text-neutral-400 text-base font-normal">Aucune</span>
                    @endif
                </p>
            </div>
            <div class="bg-white rounded-2xl p-4 border border-neutral-200 shadow-sm">
                <p class="text-xs text-neutral-500">Restaurant favori</p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
            ...
            <div class="lg:col-span-3">
                <div class="bg-white rounded-2xl border border-neutral-200 shadow-sm overflow-hidden">
                    <div class="px-5 py-4 border-b border-neutral-100 flex items-center justify-between">
                        <h3 class="font-semibold text-neutral-900">Historique des commandes</h3>
                        <span class="text-xs text-neutral-400">20 dernières</span>
                    </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-sm">
                                <thead class="bg-neutral-50 border-b border-neutral-200">
                                    <tr>
                                        <th class="px-4 py-3 text-left text-xs font-semibold text-neutral-500 uppercase">Réf.</th>
                                        <th class="px-4 py-3 text-left text-xs font-semibold text-neutral-500 uppercase">Restaurant</th>
                                        <th class="px-4 py-3 text-left text-xs font-semibold text-neutral-500 uppercase">Statut</th>
                                        <th class="px-4 py-3 text-right text-xs font-semibold text-neutral-500 uppercase">Montant</th>
                                ...
                                <tbody class="divide-y divide-neutral-100">
                                    ...
                                    <tr class="hover:bg-neutral-50/50">
                                        <td class="px-4 py-3 font-mono text-xs text-neutral-700">{{ $order->reference }}</td>
                                        <td class="px-4 py-3 text-neutral-600 text-xs">{{ $order->restaurant?->name ?? '—' }}</td>
                                        <td class="px-4 py-3">
                                            <span class="px-2 py-0.5 rounded-full text-xs font-medium {{ $s['class'] }}">{{ $s['label'] }}</span>
                                        </td>
                                        <td class="px-4 py-3 text-right font-medium text-neutral-900">{{ number_format($order->total) }} F</td>
                                        <td class="px-4 py-3 text-xs text-neutral-500">{{ $order->created_at->format('d/m/Y H:i') }}</td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
                    ...
                    @else
                        <div class="px-5 py-14 flex flex-col items-center gap-3 text-center">
                            ...
                            <p class="text-sm text-neutral-400">Aucune commande enregistrée.</p>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="lg:col-span-2 space-y-4">
                ...
                <div class="bg-white rounded-2xl border border-neutral-200 shadow-sm p-5">
                    <h3 class="font-semibold text-neutral-900 mb-3">Informations</h3>
                    <div class="divide-y divide-neutral-100 text-sm">
                        <div class="py-2 flex justify-between gap-2">
                            ...
                            <span class="font-medium text-neutral-800">{{ $customer->phone ?? '—' }}</span>
                        </div>
                        <div class="py-2 flex justify-between gap-2">
                            <span class="text-neutral-500">Ville</span>
                            <span class="font-medium text-neutral-800">{{ $customer->city ?? '—' }}</span>
                        </div>
                        <div class="py-2 flex justify-between gap-2">
                            <span class="text-neutral-500">Inscrit le</span>
                            <span class="font-medium text-neutral-800">{{ $customer->created_at->format('d/m/Y') }}</span>
                        </div>
                        <div class="py-2 flex justify-between gap-2">
                            <span class="text-neutral-500">Adresses sauvegardées</span>
                            <span class="font-medium text-neutral-800">{{ $customer->addresses->count() }}</span>
                        </div>
                        <div class="py-2 flex justify-between gap-2">
                            <span class="text-neutral-500">Token push</span>
                ...
                <div class="bg-white rounded-2xl border border-neutral-200 shadow-sm overflow-hidden">
                    <div class="px-5 py-4 border-b border-neutral-100">
                        <h3 class="font-semibold text-neutral-900">Adresses enregistrées</h3>
                    </div>
                    @if($customer->addresses->count())
                        <div class="divide-y divide-neutral-100">
                            ...
                            <div class="px-5 py-3">
                                ...
                                <p class="text-sm text-neutral-600">{{ $address->address }}</p>
                                @if($address->city)
                                    <p class="text-xs text-neutral-400 mt-0.5">{{ $address->city }}{{ $address->zone ? ' · ' . $address->zone : '' }}</p>
                                @endif
                                @if($address->instructions)
                                    <p class="text-xs text-neutral-400 mt-0.5 italic">{{ $address->instructions }}</p>
                                @endif
                            </div>
                            @endforeach
                        </div>
                    @else
                        <div class="px-5 py-8 text-center text-neutral-400 text-sm">Aucune adresse enregistrée.</div>
                    @endif
                </div>
            </div>
        </div>
