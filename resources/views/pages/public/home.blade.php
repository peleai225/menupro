<x-layouts.public title="Accueil" description="MenuPro : digitalisez votre restaurant, menu en ligne, commandes et paiement Mobile Money. Solution SaaS pour restaurants en Cote d'Ivoire.">
    ...
    <section id="pricing" class="py-20 sm:py-28 bg-white relative">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {{-- Plan GOLD — pleine largeur --}}
            <div class="mt-6 animate-on-scroll">
                <div class="relative bg-gradient-to-br from-yellow-900 via-neutral-900 to-yellow-950 rounded-2xl p-8 md:p-10 border border-yellow-600/30 shadow-2xl overflow-hidden">
                    {{-- Effet brillance --}}
                    <div class="absolute top-0 right-0 w-96 h-96 bg-yellow-500/10 rounded-full -translate-y-1/2 translate-x-1/2 blur-3xl pointer-events-none"></div>
                    <div class="absolute bottom-0 left-0 w-64 h-64 bg-yellow-600/10 rounded-full translate-y-1/2 -translate-x-1/2 blur-2xl pointer-events-none"></div>

                    <div class="relative z-10 grid md:grid-cols-2 gap-8 items-center">
                        {{-- Gauche : infos --}}
                        <div>
                            <div class="flex items-center gap-3 mb-4">
                                <span class="text-xs font-bold text-yellow-400 uppercase tracking-wider">Plan GOLD</span>
                                <span class="bg-yellow-500 text-yellow-950 text-xs font-bold px-3 py-0.5 rounded-full">Nouveau</span>
                            </div>
                            <div class="flex items-baseline gap-2 mb-2">
                                <span class="text-5xl font-bold text-white">85 000</span>
                                <span class="text-yellow-300/70 text-sm">F/mois</span>
                            </div>
                            <p class="text-yellow-100/70 text-sm mb-6">Pour les complexes, maquis multi-espaces et hôtels. Contrôle total, zéro fuite.</p>
                            <a href="https://wa.me/2250000000000?text=Je%20suis%20intéressé%20par%20le%20Plan%20GOLD%20MenuPro" target="_blank" class="inline-flex items-center gap-2 px-7 py-3.5 bg-yellow-500 text-yellow-950 font-bold rounded-xl hover:bg-yellow-400 shadow-lg shadow-yellow-500/20 transition-all hover:-translate-y-0.5">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            @foreach([
                                ['Espaces illimités (VIP, VVIP, salle...)', 'yellow'],
                                ['Caisses illimitées par espace', 'yellow'],
                                ['Serveurs avec PIN — traçabilité totale', 'yellow'],
                                ['Stock séparé par espace', 'yellow'],
                                ['Impression thermique automatique', 'yellow'],
                                ['Ticket cuisine + reçu client', 'yellow'],
                                ['Rapport de nuit par serveur', 'yellow'],
                                ['Alertes fraude et écart stock', 'yellow'],
                            ] as $f)
                            <div class="flex items-center gap-2.5 text-sm text-yellow-100/90">
                                <svg class="w-4 h-4 text-yellow-400 shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>
                                {{ $f[0] }}
                            </div>
                            @endforeach
                        </div>
