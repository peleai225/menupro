...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* SPLASH — overlay au-dessus de tout, disparaît au 1er tap */
        #splash {
        ...
        #splash h2 { font-size: 24px; font-weight: 900; color: #f97316; }
        #splash p  { font-size: 14px; color: #6b7280; }
        #splash-icon { font-size: 72px; animation: pulse 1.5s infinite; }
        ...
        #alert-overlay { display: none; position: fixed; inset: 0; background: rgba(0,0,0,.7); z-index: 150; align-items: center; justify-content: center; }
...
{{-- LIVEWIRE — toujours dans le DOM, Livewire s'initialise normalement --}}
<div id="kds-wrap">
...
{{-- SPLASH — overlay au-dessus de tout, uniquement pour déverrouiller l'audio --}}
<div id="splash">
    <div id="splash-icon">🍳</div>
    <h2>Écran Cuisine</h2>
    <p>{{ $restaurant->name }}</p>
    <button id="splash-btn" onclick="kdsUnlock()">▶ Démarrer</button>
</div>
...
(function () {
    ...
    /* ─── DÉVERROUILLAGE AUDIO ──────────────────────────────
       Appelé par le bouton splash — DANS le geste utilisateur,
       ce qui autorise AudioContext et speechSynthesis.         */
    window.kdsUnlock = function () {
        if (unlocked) return;
        unlocked = true;

        // Crée l'AudioContext dans le geste → Chrome l'autorise
        try {
            audioCtx = new (window.AudioContext || window.webkitAudioContext)();
            if (audioCtx.state === 'suspended') audioCtx.resume();
        } catch (e) {}

        // Précharge les voix TTS
        ...
        // Marque les commandes déjà présentes comme connues (pas d'alerte au démarrage)
        document.querySelectorAll('[wire\\:key^="order-"]').forEach(function (el) {
            knownIds[el.getAttribute('wire:key').replace('order-', '')] = true;
        });
    };

    /* ─── DÉTECTION NOUVELLES COMMANDES ────────────────────
       Livewire met à jour le DOM toutes les 5s (wire:poll).
       On compare les wire:key présents avec knownIds.         */
    ...
    function triggerAlert(cardEl) {
        var ref   = (cardEl.querySelector('.kds-ref')   || {}).textContent || '';
        var table = (cardEl.querySelector('.kds-table') || {}).textContent || '';
        var names = [];
        cardEl.querySelectorAll('.kds-name').forEach(function (n) {
        ...
        alertTimer = setTimeout(kdsDismissAlert, 7000);
    ...
    function kdsSpeak(ref, table, names) {
        ...
        var text = 'Nouvelle commande ' + ref.replace('#', '') + '. ';
        if (table) text += table.trim() + '. ';
        if (names.length) text += names.join(', ') + '.';
        var u = new SpeechSynthesisUtterance(text);
        u.lang = 'fr-FR'; u.rate = 0.88; u.pitch = 1.0; u.volume = 1.0;
