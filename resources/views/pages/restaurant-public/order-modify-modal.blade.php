...
<div id="modify-order-modal" 
    ...
    <div class="fixed inset-0 bg-black/50 backdrop-blur-sm transition-opacity" 
         @click="show = false; document.getElementById('modify-order-modal').classList.add('hidden');"></div>

    <!-- Modal Content -->
    <div class="relative w-full max-w-2xl mx-auto my-8 bg-white rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto"
        ...
        <div class="sticky top-0 bg-white p-6 border-b border-neutral-100 z-10">
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-xl font-bold text-neutral-900">Modifier ma commande</h2>
                    <p class="text-sm text-neutral-500 mt-1">
                        @if($order->remaining_modification_time)
                            Temps restant : {{ $order->remaining_modification_time }} minute(s)
                        @else
                            Vous pouvez modifier votre commande
                        @endif
                    </p>
                </div>
                <button @click="show = false; document.getElementById('modify-order-modal').classList.add('hidden');" 
        ...
        <div class="p-6 space-y-6">
            ...
            <!-- Current Items -->
            <div>
                <h3 class="text-lg font-semibold text-neutral-900 mb-4">Mes articles</h3>
...
<script>
    ...
    const baseUrl = `/r/${restaurantSlug}/commande/${orderToken}`;
    ...
    async function updateItemQuantity(itemId, quantity) {
        ...
        try {
            ...
            if (result.success) {
                ...
                if (result.refund) {
                    showRefundMessage(result.refund);
                }
            } else {
    ...
    // Remove item directly from list
    async function removeItem(itemId) {
        await removeItemFromModal(itemId);
    }

    // Show refund message
    function showRefundMessage(refund) {
        const message = refund.error 
            ? refund.message 
            : `Remboursement partiel de ${refund.formatted_amount} initié. Il sera traité sous 24-48h.`;
        
        const bgColor = refund.error ? 'bg-red-50 border-red-200 text-red-700' : 'bg-green-50 border-green-200 text-green-700';
        
        const messageDiv = document.createElement('div');
        messageDiv.className = `mb-4 p-4 border rounded-lg ${bgColor}`;
