        ...
        <div class="flex flex-col lg:flex-row gap-4">
            ...
            <div class="w-full lg:w-44">
                <select wire:model.live="status" class="input h-12">
                    ...
                    <option value="available">Visible sur le menu</option>
                    <option value="unavailable">Masqués du menu</option>
    ...
    @else
        ...
        <div x-show="loaded && viewMode === 'grid'" 
            ...
            @foreach($this->dishes as $index => $dish)
                <div class="card-interactive overflow-hidden group {{ !$dish->is_active ? 'ring-2 ring-neutral-300 ring-dashed opacity-70' : '' }}"
                    ...
                    <div class="aspect-square bg-gradient-to-br from-neutral-100 to-neutral-200 relative overflow-hidden">
                        ...
                        <div class="absolute top-3 left-3 flex flex-col gap-2">
                            ...
                            @endif
                            @if(!$dish->is_active)
                                <span class="inline-flex items-center gap-1 px-2.5 py-1 bg-neutral-700 text-white text-xs font-bold rounded-lg">
                            ...
                            @elseif(!$dish->is_in_stock && $dish->track_stock)
                                <span class="inline-flex items-center gap-1 px-2.5 py-1 bg-red-600 text-white text-xs font-bold rounded-lg">
                                    Épuisé
                                </span>
                            @endif
                        </div>
        ...
        <div x-show="loaded && viewMode === 'list'" 
            ...
            <div class="divide-y divide-neutral-100">
                @foreach($this->dishes as $index => $dish)
                    <div class="flex items-center gap-4 p-4 hover:bg-neutral-50 transition-colors group {{ !$dish->is_active ? 'opacity-50' : '' }}"
                        ...
                        <div class="text-right flex-shrink-0">
                            ...
                            @if(!$dish->is_active)
                                <span class="text-xs font-medium text-neutral-500 flex items-center gap-1">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029"/></svg>
                                    Masqué
                                </span>
                            @endif
                        </div>
