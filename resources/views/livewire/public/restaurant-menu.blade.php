...
<div class="min-h-screen bg-neutral-50" wire:poll.10s="refreshRestaurant">
    ...
    <!-- Restaurant Header - Bannière et infos réorganisées -->
    <header class="relative">
        {{-- Bannière : ratio 21/9 pour un rendu cohérent, object-position center pour cadrage --}}
        <div class="relative w-full aspect-[21/9] min-h-[180px] max-h-[320px] bg-neutral-800 overflow-hidden">
        ...
        <!-- Carte restaurant : logo + infos alignés horizontalement -->
        <div class="max-w-5xl mx-auto px-4 -mt-12 md:-mt-16 relative z-10">
            <div class="bg-white rounded-2xl shadow-xl shadow-neutral-900/15 ring-1 ring-neutral-200/80 p-5 md:p-6 flex flex-col sm:flex-row gap-4 sm:gap-6 items-start sm:items-center overflow-hidden relative">
                <!-- Subtle accent strip -->
                <div class="absolute top-0 left-0 right-0 h-0.5 rounded-t-2xl" style="background: linear-gradient(90deg, {{ $primaryColor }}, {{ $primaryColor }}80, transparent);"></div>
                {{-- Logo --}}
                <div class="flex-shrink-0">
                    @if($restaurant->logo_path)
                        <img src="{{ $restaurant->logo_url }}" 
                             alt="{{ $restaurant->name }}" 
                             class="w-20 h-20 sm:w-24 sm:h-24 rounded-xl border-2 border-white shadow-lg object-cover bg-white">
                    @else
                        <div class="w-20 h-20 sm:w-24 sm:h-24 rounded-xl border-2 border-white shadow-lg flex items-center justify-center" style="background: linear-gradient(135deg, {{ $primaryColor }}, {{ $secondaryColor }});">
                    ...
                    <div class="flex items-start justify-between gap-4">
                            <div>
                                <div class="flex items-center gap-2 flex-wrap">
                                    <h1 class="text-2xl md:text-3xl font-bold text-neutral-900 font-display tracking-tight">
                                        {{ $restaurant->name }}
                                    </h1>
    ...
    <nav class="sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-neutral-200/60 shadow-sm mt-8">
        <div class="max-w-5xl mx-auto px-4">
            <div class="flex items-center justify-between gap-4 py-3">
                <div class="flex items-center gap-1.5 overflow-x-auto scrollbar-hide flex-1 pb-0.5">
                <button wire:click="setCategory(null)"
                        class="flex-shrink-0 px-4 py-2 rounded-full text-sm font-semibold transition-all duration-200 whitespace-nowrap {{ !$activeCategory ? 'text-white shadow-md' : 'bg-neutral-100 text-neutral-600 hover:bg-neutral-200 hover:text-neutral-800' }}"
                        @if(!$activeCategory) style="background-color: {{ $primaryColor }}; box-shadow: 0 4px 12px {{ $primaryColor }}40;" @endif>
                    Tout
                </button>
                @foreach($this->categories as $category)
                    <button wire:click="setCategory({{ $category->id }})"
                            class="flex-shrink-0 px-4 py-2 rounded-full text-sm font-semibold transition-all duration-200 whitespace-nowrap {{ $activeCategory === $category->id ? 'text-white shadow-md' : 'bg-neutral-100 text-neutral-600 hover:bg-neutral-200 hover:text-neutral-800' }}"
            ...
            <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
                @foreach($this->dishes as $dish)
                    <div wire:click="openDish({{ $dish->id }})"
                         class="group bg-white rounded-2xl overflow-hidden shadow-sm border border-neutral-200/60 hover:shadow-lg hover:border-neutral-300/40 hover:-translate-y-1 transition-all duration-300 cursor-pointer {{ !$dish->is_available ? 'opacity-60' : '' }}">
                        <!-- Image -->
                        <div class="aspect-[4/3] relative overflow-hidden"
                            ...
                            @else
                                <div class="w-full h-full flex items-center justify-center"
                                     style="background: linear-gradient(135deg, {{ $primaryColor }}18 0%, {{ $primaryColor }}30 100%);">
                                    <svg class="w-14 h-14 opacity-40" fill="none" stroke="{{ $primaryColor }}" viewBox="0 0 24 24">
                            ...
                            @if($dish->is_featured)
                                <div class="absolute top-3 left-3 z-20">
                                    <span class="px-2.5 py-1 text-white rounded-full text-xs font-bold flex items-center gap-1 shadow-md" style="background-color: {{ $primaryColor }};">
                            ...
                            @if(!$dish->is_available)
                                <div class="absolute inset-0 bg-black/50 backdrop-blur-[1px] flex items-center justify-center z-20">
                                    <span class="px-3 py-1.5 bg-neutral-900/80 text-white rounded-full text-sm font-medium flex items-center gap-1.5 shadow-lg">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                                        </svg>
                                        Indisponible
                        ...
                        <!-- Content -->
                        <div class="p-4">
                            <h3 class="font-semibold text-neutral-900 line-clamp-1 font-display text-base">{{ $dish->name }}</h3>
                            @if($dish->description)
                                <p class="text-sm text-neutral-500 line-clamp-2 mt-1 leading-relaxed">{{ $dish->description }}</p>
                            @endif
    ...
    @if($this->cartItemsCount > 0)
        <button wire:click="toggleCart" 
                class="fixed bottom-6 right-6 z-30 flex items-center gap-3 px-5 py-4 text-white rounded-2xl shadow-2xl transition-all transform hover:scale-105 active:scale-95 lg:hidden"
    ...
    <!-- Footer -->
    <footer class="bg-white/80 backdrop-blur-sm border-t border-neutral-200/80 mt-20">
