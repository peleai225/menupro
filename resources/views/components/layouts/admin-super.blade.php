<x-layouts.app :title="($title ?? 'Administration') . ' - MenuPro Admin'">
    <div x-data="sidebar()" class="min-h-screen bg-neutral-50">
                ...
                <nav class="flex-1 overflow-y-auto py-6 px-3 space-y-2">
                    ...
                    <a href="{{ route('super-admin.restaurants.index') }}" 
                       class="sidebar-item {{ request()->routeIs('super-admin.restaurants*') ? 'sidebar-item-active' : '' }}">
                        ...
                        <span class="sidebar-badge-restaurants ml-auto bg-primary-500 text-white text-xs px-2 py-0.5 rounded-full {{ ($pendingRestaurants ?? 0) > 0 ? '' : 'hidden' }}" x-show="expanded">{{ $pendingRestaurants ?? 0 }}</span>
                    </a>
                    ...
                    <a href="{{ route('super-admin.commando.agents.index') }}" 
                       class="sidebar-item {{ request()->routeIs('super-admin.commando*') ? 'sidebar-item-active' : '' }}">
                        <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4M5.121 17.804A13.937 13.937 0 0112 16c2.5 0 4.847.655 6.879 1.804M15 10a3 3 0 11-6 0 3 3 0 016 0zm6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                        </svg>
                        <span x-show="expanded" x-transition class="whitespace-nowrap">Commando</span>
                        <span class="sidebar-badge-commando ml-auto bg-orange-500 text-white text-xs px-2 py-0.5 rounded-full {{ ($pendingCommandoAgents ?? 0) > 0 ? '' : 'hidden' }}" x-show="expanded">{{ $pendingCommandoAgents ?? 0 }}</span>
                    </a>
        ...
        <aside x-show="mobileOpen" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="-translate-x-full" x-transition:enter-end="translate-x-0" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="translate-x-0" x-transition:leave-end="-translate-x-full" class="fixed left-0 top-0 h-full w-64 bg-white border-r border-neutral-200 z-40 lg:hidden flex flex-col" x-cloak>
            ...
            <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-1" @click="mobileOpen = false">
                ...
                <a href="{{ route('super-admin.restaurants.index') }}" class="sidebar-item {{ request()->routeIs('super-admin.restaurants*') ? 'sidebar-item-active' : '' }}"><svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg><span>Restaurants</span><span class="sidebar-badge-restaurants ml-auto bg-primary-500 text-white text-xs px-2 py-0.5 rounded-full {{ ($pendingRestaurants ?? 0) > 0 ? '' : 'hidden' }}">{{ $pendingRestaurants ?? 0 }}</span></a>
                ...
                <a href="{{ route('super-admin.commando.agents.index') }}" class="sidebar-item {{ request()->routeIs('super-admin.commando*') ? 'sidebar-item-active' : '' }}"><svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4M5.121 17.804A13.937 13.937 0 0112 16c2.5 0 4.847.655 6.879 1.804M15 10a3 3 0 11-6 0 3 3 0 016 0zm6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg><span>Commando</span><span class="sidebar-badge-commando ml-auto bg-orange-500 text-white text-xs px-2 py-0.5 rounded-full {{ ($pendingCommandoAgents ?? 0) > 0 ? '' : 'hidden' }}">{{ $pendingCommandoAgents ?? 0 }}</span></a>
        ...
        <div :class="expanded ? 'lg:ml-64' : 'lg:ml-20'" class="transition-all duration-300 ml-0">
            ...
            <header class="sticky top-0 z-20 bg-white/95 backdrop-blur-md border-b border-neutral-200 shadow-sm">
                <div class="flex items-center justify-between h-14 sm:h-16 px-3 sm:px-4 lg:px-8 gap-2">
                    ...
                    <!-- Logo MenuPro (navbar) -->
                    <a href="{{ route('super-admin.dashboard') }}" class="hidden sm:flex items-center shrink-0 mr-3">
    ...
    <script>
        (function() {
            ...
            window.__superAdminBadgesUrl = url;
